...
export function renderGridHorarios(gridEl, bloques) {
  ...
  gridEl.innerHTML = bloques
    .map(({ hora, estado }, i) => {
      ...
      return `<button type="button" ${disabled ? 'disabled' : ''} data-hora="${hora}" style="animation-delay:${delay}ms" class="slot-btn opacity-0 animate-fade-in-up border ${clases} font-mono text-xs py-2.5 transition-all duration-200">${hora}</button>`;
    })
    .join('');
}
